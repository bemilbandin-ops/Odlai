...
const productCard = (product: Product): string => `
  <article class="card product-card">
    <a class="product-image" href="#/products/${product.slug}" aria-label="Visa ${escapeHtml(product.name)}">${product.image}</a>
    <div class="card-body">
      <span class="badge ${product.stockStatus}">${stockLabel(product.stockStatus)}</span>
      ${product.beginnerFriendly ? '<span class="badge soft">Nybörjarvänlig</span>' : ''}
...
const shell = (content: string): string => `
  <header class="site-header">
    <a class="brand" href="#/" aria-label="Odlai startsida"><span class="brand-mark">O</span><span><strong>Odlai</strong><small>Inomhusodling med omtanke</small></span></a>
  ...
  <footer class="site-footer">
    <div><strong>Odlai</strong><p>Utrustning för laglig, trygg och lättillgänglig växtvård inomhus.</p></div>
    ...
    <div><h2>Kontakt</h2><p>Demo: kontaktuppgifter ersätts inför lansering.</p>${link('/contact', 'Kontaktformulär')}</div>
  </footer>`;

const home = (): string => `
  <section class="hero">
    <div><p class="eyebrow">Svensk webbshop för växtvård</p><h1>Gör inomhusodling enkel, ren och inspirerande.</h1><p>Odlai samlar växtbelysning, odlingstält, ventilation, substrat och tillbehör för kryddväxter, chili, tomater, bladgrönt och prydnadsväxter.</p><div class="actions">${link('/products', 'Se produkter', 'button')} ${link('/guides', 'Läs guider', 'button secondary')}</div></div>
    <div class="hero-panel" role="img" aria-label="Illustration av växter på odlingshylla"><span>🌿</span><span>☀️</span><span>🪴</span></div>
  </section>
  <section><div class="section-heading"><p class="eyebrow">Populära kategorier</p><h2>Hitta rätt utrustning snabbare</h2></div><div class="grid categories">${getCategories().slice(0, 6).map((category) => `<article class="card category-card"><div class="emoji">${category.image}</div><h3>${link(`/categories/${category.slug}`, category.name)}</h3><p>${category.description}</p></article>`).join('')}</div></section>
  <section class="value-grid"><article><h2>Nybörjarvänligt</h2><p>Tydliga produkttexter och guider hjälper dig välja rimlig utrustning utan onödig komplexitet.</p></article><article><h2>Kontrollerad miljö</h2><p>Bygg en ren plats för ljus, luft och vattning anpassad efter vanliga växter inomhus.</p></article><article><h2>Trygg demobutik</h2><p>Kassan visar flödet utan att koppla till betalning, fraktbolag eller externa tjänster.</p></article></section>
  <section><div class="section-heading"><p class="eyebrow">Utvalt</p><h2>Startprodukter</h2></div><div class="grid products">${getFeaturedProducts().map(productCard).join('')}</div></section>
  <section class="trust"><h2>Neutral och laglig inriktning</h2><p>All kommunikation fokuserar på hobbyodling, växtvård och kontrollerad inomhusmiljö för lagliga växter.</p></section>
  <section class="newsletter"><h2>Få odlingsidéer i inkorgen</h2><p>Nyhetsbrevet är en placeholder. Ingen e-post skickas i denna demo.</p><form data-newsletter><label for="newsletter-email">E-post</label><input id="newsletter-email" type="email" required placeholder="namn@example.se"><button class="button" type="submit">Anmäl intresse</button></form><p class="form-note" aria-live="polite"></p></section>`;
...
const productsPage = (categorySlug?: string): string => {
  ...
  return `<section class="page-hero"><p class="breadcrumbs">${link('/', 'Start')} / ${category ? `${link('/categories', 'Kategorier')} / ${category.name}` : 'Produkter'}</p><h1>${title}</h1><p>${category?.description ?? 'Sök, filtrera och jämför utrustning för växter inomhus.'}</p></section>
    <section class="catalog-layout"><aside class="filters"><h2>Filtrera</h2><form data-filters action="#${action}"><label>Sök<input name="q" value="${escapeHtml(params.get('q') ?? '')}" placeholder="Sök produkter"></label><label>Minpris<input name="min" type="number" min="0" value="${escapeHtml(params.get('min') ?? '')}"></label><label>Maxpris<input name="max" type="number" min="0" value="${escapeHtml(params.get('max') ?? '')}"></label><label>Sortera<select name="sort"><option value="featured">Utvalt</option><option value="price-asc">Pris stigande</option><option value="price-desc">Pris fallande</option><option value="name">Namn</option></select></label><label class="check"><input name="beginner" type="checkbox" ${params.get('beginner') === '1' ? 'checked' : ''}> Nybörjarvänlig</label><label class="check"><input name="stock" type="checkbox" ${params.get('stock') === '1' ? 'checked' : ''}> I lager</label><button class="button" type="submit">Uppdatera</button>${link(action, 'Rensa filter', 'button ghost')}</form></aside><div><p>${products.length} produkter visas</p>${products.length ? `<div class="grid products">${products.map(productCard).join('')}</div>` : '<div class="empty"><h2>Inga produkter hittades</h2><p>Prova bredare sökning eller ta bort något filter.</p></div>'}</div></section>`;
};
